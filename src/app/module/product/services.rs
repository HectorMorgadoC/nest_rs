pub(crate) mod service {    
    ...
    impl Service {
        ...
        pub(crate) async  fn update(&self, _dto: &Product, _id: &Uuid) -> Result<String,ProblemDetails> {
            
            match self.repository.update(_dto, _id) {
                Ok(1) => Ok(format!("Updated product: {}",_id.clone())) ,
                Ok(0) => Err(ProblemDetails::not_found()) ,
                Ok(_) => Err(ProblemDetails::conflict("Update affected rows; expected 1".to_string())),
                Err(err) => {
